...
from position_confix import Vps, Hmatrixs
...
def main(opt, filename, update_video):
    ...
    homo_matrix = Hmatrixs[filename.split("/")[-2]]['H']
            ...
            if det is not None and len(det):
                ...
                if len(output_track) > 0:
                    ...
                    online_im = plot_tracking(
                        im0, online_tlwhs, online_ids, frame_id=frame_id + 1, fps=1. / (time.time() - startTime)
                    )
                    # DEBUG Wait for pose thread
                    pose_thread.join()
...
def speedEstimationAll_1(update_status):
    print("Created thread 1")
    opt = make_parser()
    vid_list = []
    vid_path = '../Datasets/brnocompspeed/data/2016-ITS-BrnoCompSpeed/dataset'
    for i in range(1, 7):
        dir_list = ['session{}_left'.format(i), 'session{}_right'.format(i), 'session{}_center'.format(i)]
        vid_list.extend([os.path.join(vid_path, d, 'video.avi') for d in dir_list])
    for filename in vid_list:
        main(opt, filename, update_status)
    # update_status.emit(True)
